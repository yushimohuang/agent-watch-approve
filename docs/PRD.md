...
当 Cursor / Trae / Claude Code 等 AI 工具执行敏感操作（如 `npm install`、`git push`、`rm -rf`）时，Agent Watch **实时拦截**这些操作，**推送带"批准/拒绝"按钮的卡片到飞书 App**，让用户在任何地方（手机 / 手表 / Mac / Windows）**一键决策**。
...
- **支持 Agent**（10+）：
  - 海外：Cursor / Claude Code / Codex / Gemini CLI
  - 国产：Trae / Codebuddy / Comate / Mimo / Qoder-CN / MiniMax
