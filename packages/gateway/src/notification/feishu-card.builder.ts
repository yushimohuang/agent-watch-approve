 ...
 * - url 按钮（跳转链接）：手表上点击后直接打开浏览器/Dashboard，兼容性最好
 *
 * 策略：同时放 callback + url 两种按钮
 * - 手机/PC：点 callback 按钮 → webhook 回调 → 即时更新卡片
 * - 手表：收到飞书系统通知 → 点通知打开飞书 → 看到卡片 → 点 url 按钮跳转 Dashboard
 */
 ...
 * - action 2：url 按钮（手表快捷操作）— 跳转到 Dashboard 审批页
 * - note：过期时间 + 会话 ID
 *
 * 手表弹窗流程：
 * 1. 飞书推送卡片 → 飞书 App 收到消息 → 系统通知弹窗（手表可见）
 * 2. 手表上看到通知摘要 → 点通知 → 打开飞书卡片
 * 3. 点 url 按钮 → 跳转 Dashboard 审批页 → 在 Dashboard 上操作
 */
export function buildApprovalCard(params: {
  ...
  // Dashboard 审批详情页 URL
  const approvalUrl = params.detailUrl || `${config.dashboardUrl}/approvals/${params.approvalId}`;
  ...
  // URL 按钮 - 跳转模式（手表/任何设备点击后打开 Dashboard）
  // 注：v2.1 安全增强（双保险）：
  //   1. URL 带一次性 token（5 分钟过期，HMAC 签名）
  //   2. confirm=1 跳过 Dashboard 中转页（飞书手表浏览器体验差，直接决策）
