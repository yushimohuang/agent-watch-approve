...
const AGENT_WATCH_MCP_DIR = path.join(os.homedir(), '.agent-watch-approve/comate-mcp');
...
const GATEWAY_URL = process.env.AGENT_WATCH_APPROVE_GATEWAY || '${config.gatewayUrl}';
const USER_ID = process.env.AGENT_WATCH_APPROVE_USER || '${config.userId}';
const TIMEOUT = (parseInt(process.env.AGENT_WATCH_APPROVE_TIMEOUT) || ${config.approvalTimeout || 60}) * 1000;
