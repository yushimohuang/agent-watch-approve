...
`$env:AGENT_WATCH_SESSION_ID = '$SessionId'
try {
    `$proc = Start-Process -FilePath 'powershell.exe' -ArgumentList '-ExecutionPolicy','Bypass','-File',`"`$PSScriptRoot\trae-process-monitor.ps1`",'-GatewayUrl','$GatewayUrl','-UserId','$UserId','-SessionId',`"`$env:AGENT_WATCH_SESSION_ID`",'-ApproveTimeoutSeconds','$ApproveTimeoutSeconds' -PassThru -WindowStyle Hidden
    Write-Host '[Agent Watch] Monitor started (PID: '`$proc.Id')'
} catch {
    Write-Host '[Agent Watch] Monitor start failed: '`$($_.Exception.Message)
}
# === /Agent Watch ===
"@
