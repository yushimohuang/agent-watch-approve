 ...
 *   - POST /v1/approvals/:approvalId          -> submit decision (飞书 / mobile)
 *
 ...
 *   - 飞书 / mobile still use the HTTP routes below to submit the decision;
...
// v1 hook CLI: find existing pending approval or create new one (deduplication)
router.post('/find-or-create', AuthMiddleware.requireAuth, ApprovalsController.findOrCreate);

// v1 CLI: poll for decision
router.get('/:approvalId/status', AuthMiddleware.requireAuth, ApprovalsController.getStatus);

// 飞书 / mobile: list pending
router.get('/pending', AuthMiddleware.requireAuth, ApprovalsController.getPending);

// 飞书 / mobile / push action button: submit decision
