 ...
 * - Dashboard 启动时若没 token → 自动调 /v1/auth/auto-anonymous 拿 token → 直接进
 * - 公网暴露（PUBLIC_URL 不为空）→ 要求设置 DASHBOARD_PASSWORD → /v1/auth/check-password 校验
 */
...
export const AuthController = {
   ...
   *
   * 仅在 PUBLIC_URL 非空（即公网暴露）时启用
   * - 不设 DASHBOARD_PASSWORD = 拒绝公网访问（fail-closed）
   * - 设了 DASHBOARD_PASSWORD = 校验返回 token
   */
  async checkDashboardPassword(req: Request, res: Response) {
    const isPublicExposed = !!config.publicUrl && !config.publicUrl.includes('localhost');
    if (!isPublicExposed) {
      // 本地模式：直接 anonymous 登录即可
    ...
    if (!expected) {
      return res.status(503).json({
        error: {
          ...
          message: '检测到公网暴露但未设置 DASHBOARD_PASSWORD，请在 .env 中配置后重启',
        },
        success: false,
      });
    }
    ...
    // 密码正确 → 走 anonymous 登录拿 token
    return AuthController.autoAnonymous(req, res);
  },

  /**
   * [v2.1 本地优先] 返回当前模式 + 是否需要密码
   */
  async getAuthMode(req: Request, res: Response) {
    ...
    res.json({
      data: {
        ...
        passwordSet: !!config.dashboardPassword,
