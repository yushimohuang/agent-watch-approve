...
(async () => {
  ...
  if (isPublicExposed && !process.env.ACCESS_PASSWORD) {
    logger.warn(
      'PUBLIC_URL exposes Gateway publicly, but ACCESS_PASSWORD is not set. Public access will return 503. Set ACCESS_PASSWORD in .env to enable.',
    );
  }
  ...
  if (!process.env.LOCAL_USER_NAME && process.stdin.isTTY && !isPublicExposed) {
    ...
    rl.question(
      `\n[Agent Watch Approve] 本地用户名（直接回车用默认: "${localUser.displayName}"）：`,
      (answer: string) => {
        const name = answer.trim() || localUser.displayName;
        if (name !== localUser.displayName) {
          setLocalUserName(name);
          logger.info('Local user name updated', { name });
          console.log(`\n[Agent Watch Approve] ✓ 本地用户名已设置为: ${name}\n`);
        } else {
          console.log(`\n[Agent Watch Approve] ✓ 使用默认用户名: ${name}\n`);
        }
        rl.close();
      },
    );
  ...
  server.listen(PORT, () => {
    logger.info(`Agent Watch Approve Gateway started`, {
      port: PORT,
