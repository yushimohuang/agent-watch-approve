...
interface ConnectionInfo {
  ...
  subscribedApprovals: Set<string>;
  connectedAt: Date;
  lastHeartbeat: Date;
  ipAddress: string;
  ws: WebSocket;
...
export class WebSocketHandler {
    ...
    try {
      ...
      switch (message.type) {
        ...
        case 'approval_subscribe':
          this.handleApprovalSubscribe(connection, message.payload);
          break;
        case 'ping':
  ...
  private handleApprovalSubscribe(connection: ConnectionInfo, payload: any): void {
    const { approvalId, sessionId } = payload;
    // Track which approvals this connection is subscribed to
    if (!connection.subscribedApprovals) connection.subscribedApprovals = new Set();
    if (approvalId) connection.subscribedApprovals.add(approvalId);
    if (sessionId) connection.sessionIds.add(sessionId);

    logger.info('Approval subscription registered', {
