/**
 * E2E test: prove that when user denies an approval via Feishu,
...
async function main() {
  ...
  // 2. 写一个行内的 "fake agent" 脚本到 /tmp（子进程会执行它发送 item.completed）
  const fakeAgentPath = join(process.cwd(), '.fake-agent.tmp.cjs');
  const fakeAgentSrc = `
const http = require('http');
process.on('SIGINT', () => { console.log('[fake-agent] received SIGINT'); process.exit(0); });
setInterval(() => console.log('[fake-agent] heartbeat'), 500);
setTimeout(() => {
  // 写到 stdout，让 HookManager parse 成 item.completed command_execution
  process.stdout.write(JSON.stringify({
    type: 'item.completed',
    payload: { command: 'rm -rf /tmp/test', cwd: process.cwd() }
  }) + '\\n');
}, 500);
`;
  writeFileSync(fakeAgentPath, fakeAgentSrc);
  const tsxBin = join(__dirname, '../../../node_modules/.bin/tsx.cmd');
  // 用 node 直接跑 .cjs（避免 tsx 启动慢）
  const nodeBin = process.platform === 'win32' ? 'node.exe' : 'node';

  console.log('[test] starting HookManager with fake-agent subprocess...');
  const hookManager = new HookManager({
    agentType: 'codex',
    agentCommand: [nodeBin, fakeAgentPath],
