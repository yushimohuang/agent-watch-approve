...
// 默认配置
const DEFAULT_GATEWAY = process.env.AGENT_WATCH_GATEWAY || 'http://localhost:3000';
const DEFAULT_TIMEOUT = 60;
const ACCESS_TOKEN = process.env.AGENT_WATCH_TOKEN || '';
...
function parseArgs() {
  const args = {
    ...
    user: process.env.AGENT_WATCH_USER || '',
    tool: 'unknown',
    timeout: DEFAULT_TIMEOUT,
    sessionId: process.env.AGENT_WATCH_SESSION || '',
    cwd: process.cwd(),
    extras: {},
  };
...
async function approve() {
  ...
  const approvalRequest = {
    ...
    platform: stdinData.platform || process.env.AGENT_WATCH_PLATFORM || 'unknown',
...
switch (args.command || 'approve') {
  case 'approve':
    ...
    break;
  case 'version':
    console.log('agent-watch CLI v1.0.0');
    break;
  case 'help':
